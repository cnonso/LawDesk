...
namespace LawDesk.Models
{
    ...
    public class AppHandler
    {
        ...
        {
            double dec = 0.0;
            double.TryParse(str.ToString(), out dec);
            return dec;
        }
        ...
        public static string GetPreviousMonth(string month)
        {
            ...
            else if (month == "February") return "January";
            else if (month == "March") return "February";
            else if (month == "April") return "April";
            else if (month == "May") return "April";
            else if (month == "June") return "May";
            else if (month == "July") return "June";
            else if (month == "August") return "July";
            else if (month == "September") return "August";
            else if (month == "October") return "September";
            else if (month == "November") return "October";
            else if (month == "December") return "November";
            return "";
        }
        ...
        public static bool InArray(string key, string[] arr)
        {
        ...
        public static string ToDMY(DateTime dt, bool fullmonth)
        {
            return dt.Day + "-" + (!fullmonth ? MonthToString(dt.Month).Substring(0, 3) : MonthToString(dt.Month)) + "-" + dt.Year;
        }
        ...
        public static bool LoginUploader(string password)
        {
            return Encrypt(password) == "PU0wYmtWMlZoSm5jcDltYw==";
        }
        ...
        public static bool ZipDirectory(string zipFileToCreate, string directoryToZip)
        {
            ZipFileError = "";
            try
            {
                ...
                {
                    zip.TempFileFolder = directoryToZip;
            ...
            {
                ZipFileError = ex1.Message;
            }
            return false;
        }

        public static bool CreateZipFile(string fileToZip, string zipFileToCreate, string directoryToZip)
        {
            ZipFileError = "";
            string[] archiveName = zipFileToCreate.Replace(".zip", "").Replace("\\", "/").Split('/');
            try
            {
                using (ZipFile zip = new ZipFile())
                {
                    zip.TempFileFolder = directoryToZip;
                    zip.StatusMessageTextWriter = System.Console.Out;
                    zip.AddFile(fileToZip, archiveName[archiveName.Length - 1]);
                    zip.Save(zipFileToCreate);
                    return true;
                }
            }
        ...
        public static string[] DatatableToArray(string fieldName, DataTable table)
        {
            ArrayList strValues = new ArrayList();
            foreach (DataRow dr in table.Rows)
                strValues.Add(dr[fieldName].ToString());
            return (string[])strValues.ToArray(typeof(string));
        }
        ...
        public static object[] DataTableToArray(DataTable table, string column)
        {
        ...
        {
            string[] pageName = HttpContext.Current.Request.Url.AbsolutePath.Split('/');
